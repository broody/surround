...
const move = (s, kind, point = p.NO_POINT, dead = 0n) => s.move(p.goStep(s.due(), kind, point, dead), keys[s.due()]);
const span = xs => [BigInt(xs.length), ...xs];

/** Calldata for offchain/proving: terms, start, history witness, signed steps. */
...
use referee::{Envelope, SignedStep, Terms};
...
pub struct Vector {
    ...
    pub steps: Span<SignedStep<GoAction>>,
...
fn vector(mut input: Span<felt252>, end_hash: felt252) -> Vector {
    ...
    let steps = Serde::<Span<SignedStep<GoAction>>>::deserialize(ref input).unwrap();
    assert!(input.is_empty());
    Vector { terms, start, history, steps, end_hash }
}
